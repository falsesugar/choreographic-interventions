...
function bodyTracked(body) {
  ...
  // Draw a line
  stroke(255);
    line(nose.x, nose.y, pelvis.x, pelvis.y);
    line

  // Draw a curve
  noFill();
  beginShape();
  curveVertex(shoulderRight.x, shoulderRight.y)
  curveVertex(shoulderLeft.x, shoulderLeft.y);
  curveVertex(elbowRight.x, elbowRight.y);
  curveVertex(elbowLeft.x, elbowLeft.y);
  curveVertex(wristRight.x, wristRight.y);
  curveVertex(wristLeft.x, wristLeft.y);
  endShape(CLOSE);
}
